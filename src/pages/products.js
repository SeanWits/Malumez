...
const Products = ({ user }) => {
        ...
        if (filteredPopulated) {
            ...
            const handleSearchReceivedChange = (newValue) => {
                ...
                if (filterClicked === false) {
                    // this only runs if the filter hasn't been clicked
                    if (searchReceived === "nothing") {
                        // if there is nothing in the search, display all products
                        products.forEach((product) => {
                            someProducts[i] = product;
                            i++;
                        });
                    } else {
                        // the searchReceived is populated
                        products.forEach((product) => {
                            if (
                                (product.brand &&
    ...
    async function fetchProducts() {
        try {
            ...
            const productPromises = shopQuerySnapshot.docs.map(
                async (shopDoc) => {
            ...
            async function checkProducts() {
                if (products.length === 0) {
                    ...
                    if (
                        searchReceived.length === 0 ||
                        searchReceived === "nothing"
                    ) {
                        ...
                        for (let i = 0; i < allProducts.length; i++) {
                            if (
                                (allProducts[i].brand &&
                                    allProducts[i].brand.toLowerCase() ===
                                        searchReceived.toLowerCase()) ||
                                (allProducts[i].category &&
                                    allProducts[i].category.toLowerCase() ===
                                        searchReceived.toLowerCase()) ||
                                (allProducts[i].name &&
                                    allProducts[i].name
                                        .toLowerCase()
                                        .includes(searchReceived.toLowerCase()))
                            ) {
                                someProducts[i] = allProducts[i];
                            }
                        }
